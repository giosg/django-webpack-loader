...
* Merged forks https://github.com/owais/django-webpack-loader and https://github.com/alihazemfarouk/django-webpack-loader
* Fixed tests
* Made some code style fixes
* Added notes and documentation that `webpack4-bundle-tracker` is required in order of `render_entrypoint` to work.

* Tested `render_entrypoint` to be compatible with following versions:
  * `webpack@4.11.1` and `webpack@4.40.2`
  * `webpack4-bundle-tracker@0.0.1-beta`

**NOTE:** Using `render_entrypoint` wont work if you are using `webpack-bundle-tracker` instead of `webpack4-bundle-tracker`.
...
**NOTE:** Using `render_entrypoint` requires usage of Webpack 4 or newer and `webpack4-bundle-tracker` instead of `webpack-bundle-tracker`.
